...
app.post('/api/restaurant/menu/categories/reorder',requireRestaurantDashboard,async(req,res)=>{const client=await db.connect();try{const rows=Array.isArray(req.body?.categories)?req.body.categories.slice(0,200):[];await client.query('BEGIN');for(let i=0;i<rows.length;i++){const id=Number(rows[i]?.id);if(id)await client.query('UPDATE menu_categories SET sort_order=$1,updated_at=NOW() WHERE id=$2',[i,id]);}for(let i=0;i<rows.length;i++){const id=Number(rows[i]?.id);if(!id)continue;const q=await client.query('SELECT name_ar FROM menu_categories WHERE id=$1',[id]);if(q.rowCount){const cat=q.rows[0].name_ar;await client.query('UPDATE menu_item_overrides SET category_sort_order=$1,updated_at=NOW() WHERE category=$2',[i,cat]);for(const item of loadStaticMenuCatalog().filter(x=>x.category===cat)){await client.query(`INSERT INTO menu_item_overrides(item_key,category,category_sort_order,is_custom) VALUES($1,$2,$3,FALSE) ON CONFLICT(item_key) DO UPDATE SET category_sort_order=EXCLUDED.category_sort_order,updated_at=NOW()`,[item.itemKey,cat,i]);}}}await client.query('COMMIT');staticMenuCatalogCache=null;res.json({ok:true})}catch(e){await client.query('ROLLBACK').catch(()=>{});console.error('Category reorder error',e);res.status(500).json({ok:false,message:'تعذر حفظ ترتيب الأقسام.'})}finally{client.release()}});
...
app.post("/api/restaurant/menu/save", requireRestaurantDashboard, async (req,res)=>{
  try{
    ...
    if(!p.nameAr) return res.status(400).json({ok:false,message:'اسم الصنف بالعربية مطلوب.'});
...
app.post('/api/restaurant/menu/reorder',requireRestaurantDashboard,async(req,res)=>{const client=await db.connect();try{const rows=Array.isArray(req.body?.items)?req.body.items.slice(0,500):[];if(!rows.length)return res.json({ok:true});const category=cleanMenuField(rows[0]?.category,120);const cq=await client.query('SELECT sort_order FROM menu_categories WHERE name_ar=$1',[category]);const categorySortOrder=Number(cq.rows[0]?.sort_order||0);await client.query('BEGIN');for(let i=0;i<rows.length;i++){const key=cleanMenuField(rows[i]?.itemKey,200);if(!key)continue;await client.query(`INSERT INTO menu_item_overrides(item_key,category,sort_order,category_sort_order,is_custom) VALUES($1,$2,$3,$4,$5) ON CONFLICT(item_key) DO UPDATE SET category=EXCLUDED.category,sort_order=EXCLUDED.sort_order,category_sort_order=EXCLUDED.category_sort_order,updated_at=NOW()`,[key,category,i,categorySortOrder,key.startsWith('custom:')]);}await client.query('COMMIT');staticMenuCatalogCache=null;res.json({ok:true})}catch(e){await client.query('ROLLBACK').catch(()=>{});console.error('Item reorder error',e);res.status(500).json({ok:false,message:'تعذر حفظ ترتيب الأصناف.'})}finally{client.release()}});
