...
- In a negative pre-order phrase, STT may write "بدون طلب مصطفى" when the guest said "بدون طلب مسبق". Treat it as preorder_choice=no and do not change the remembered guest name unless the latest turn contains an explicit naming cue such as "اسمي", "باسم", or "الاسم".
- In an active booking, rushed phrases such as "لثلاثة", "ثلاثة شخاص", or "ثلاثة أشخاص" all clearly mean party_size=3. Apply the same contextual reasoning to other party numbers. Extract every clear fact even if a different field in the same sentence looks like test data or is invalid.
